...
Func CheckWall()
	;$g_iCmbUpgradeWallsLevel = _GUICtrlComboBox_GetCurSel($cmbWalls)
	...
	For $i = 0 To UBound($listLvlMask) - 1
		...
		Local $result = DllCall($g_hLibFunctions, "str", "findWall", "ptr", $hHBitmap2, "double", $listLvlMask[$i], "int", $levelWall, "double", $Walltolerance[$g_iCmbUpgradeWallsLevel], "int", $iMaxNbWall, "int", $g_iDebugWalls)
	...
	Else
		...
		For $i = 0 To UBound($listPixel) - 1
			...
			For $j = 0 To 1 ; try compensation
				...
				If _Sleep(500) Then Return
				$aResult = BuildingInfo(245, 520 + $g_iBottomOffsetY) ; Get Unit name and level with OCR 860x780
				If $aResult[0] = 2 Then ; We found a valid building name
					If StringInStr($aResult[1], "wall") = True And Number($aResult[2]) = ($g_iCmbUpgradeWallsLevel + 4) Then ; we found a wall
						Setlog("Position No: " & $i + 1 & " is a Wall Level: " & $g_iCmbUpgradeWallsLevel + 4 & ".")
						Return True
					Else
						If $g_iDebugSetlog Then
							ClickP($aAway, 1, 0, "#0931") ;Click Away
							Setlog("Position No: " & $i + 1 & " is not a Wall Level: " & $g_iCmbUpgradeWallsLevel + 4 & ". It was: " & $aResult[1] & ", " & $aResult[2] & " at: (" & $pixel[0] + $xCompensation & "," & $pixel[1] + $yCompensation & ").", $COLOR_DEBUG) ;debug
						Else
							ClickP($aAway, 1, 0, "#0932") ;Click Away
							Setlog("Position No: " & $i + 1 & " is not a Wall Level: " & $g_iCmbUpgradeWallsLevel + 4 & ".", $COLOR_ERROR)
						EndIf
					EndIf
				Else
