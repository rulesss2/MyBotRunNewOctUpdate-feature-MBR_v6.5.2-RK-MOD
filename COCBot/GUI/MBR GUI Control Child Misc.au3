...
Func cmbProfile()
	...
	FileClose($g_hhLogFileHandle)
	$g_hhLogFileHandle = ""		;- Writing log for each profile in SwitchAcc Mode - DEMEN (Special thanks to ezeck0001)
	FileClose($g_hhAttackLogFileHandle)
	$g_hhAttackLogFileHandle = ""	;- Writing log for each profile in SwitchAcc Mode - DEMEN (Special thanks to ezeck0001)
...
Func btnAddConfirm()
	Switch @GUI_CtrlId
		...
		Case $g_hBtnConfirmAddProfile
			...
			;====== SwitchAcc - DEMEN ==============
			Local $g_hiNewProfile = _GUICtrlCombobox_GetCurSel($g_hcmbProfile)
			Local $g_hUpdatedProfileList = _GUICtrlComboBox_GetListArray($g_hcmbProfile)
			Local $g_hnUpdatedTotalProfile = _GUICtrlComboBox_GetCount($g_hcmbProfile)

			If $g_hiNewProfile <= 7 Then
				_GUICtrlComboBox_SetCurSel($g_hcmbAccountNo[$g_hiNewProfile], -1)		; clear config of new profile
				_GUICtrlComboBox_SetCurSel($g_hcmbProfileType[$g_hiNewProfile], -1)
				For $i = 7 To $iNewProfile+1  Step -1
					_GUICtrlComboBox_SetCurSel($g_hcmbAccountNo[$i], $g_haMatchProfileAcc[$i-1]-1)	; push config up 1 level. -1 because $aMatchProfileAcc is saved from 1 to 8
					_GUICtrlComboBox_SetCurSel($g_hcmbProfileType[$i], $g_haProfileType[$i-1]-1)
				Next
			EndIf
			btnUpdateProfile()
			;====== SwitchAcc - DEMEN ==============
...
Func btnDeleteCancel()
	Switch @GUI_CtrlId
		Case $g_hBtnDeleteProfile
			...
			;====== SwitchAcc - DEMEN ==============
			saveConfig()
			Local $g_hiDeleteProfile = _GUICtrlCombobox_GetCurSel($g_hcmbProfile)
			;====== SwitchAcc - DEMEN ==============
			...
			EndIf
				...
				If $g_hiDeleteProfile <= 7 Then
					For $i = $g_hiDeleteProfile To 7
						If $i <=6 Then
							_GUICtrlComboBox_SetCurSel($g_hcmbAccountNo[$i], $g_haMatchProfileAcc[$i+1]-1)
							_GUICtrlComboBox_SetCurSel($g_hcmbProfileType[$i], $g_haProfileType[$i+1]-1)
						Else
							_GUICtrlComboBox_SetCurSel($g_hcmbAccountNo[$i], -1)
							_GUICtrlComboBox_SetCurSel($g_hcmbProfileType[$i], -1)
						EndIf
					Next
				EndIf
				btnUpdateProfile()
