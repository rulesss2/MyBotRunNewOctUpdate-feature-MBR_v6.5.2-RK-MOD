...
Func UpdateStats()
	Local Static $s_iOldSmartZapGain = 0, $s_iOldNumLSpellsUsed = 0, $s_iOldNumEQSpellsUsed = 0
	Local $bDonateTroopsStatsChanged = False, $bDonateSpellsStatsChanged = False

	Local Static $iOldFreeBuilderCount = 0, $iOldTotalBuilderCount = 0, $iOldGemAmount = 0 ; builder and gem amounts
	Local Static $iOldGoldCurrent = 0, $iOldElixirCurrent = 0, $iOldDarkCurrent = 0, $iOldTrophyCurrent = 0 ; current stats
	...
	Local Static $iOldSkippedVillageCount = 0, $iOldDroppedTrophyCount = 0 ; skipped village and dropped trophy counts
	Local Static $iOldCostGoldWall = 0, $iOldCostElixirWall = 0, $iOldCostGoldBuilding = 0, $iOldCostElixirBuilding = 0, $iOldCostDElixirHero = 0 ; wall, building and hero upgrade costs
	Local Static $iOldNbrOfWallsUppedGold = 0, $iOldNbrOfWallsUppedElixir = 0, $iOldNbrOfBuildingsUppedGold = 0, $iOldNbrOfBuildingsUppedElixir = 0, $iOldNbrOfHeroesUpped = 0 ; number of wall, building, hero upgrades with gold, elixir, delixir
	Local Static $iOldSearchCost = 0, $iOldTrainCostElixir = 0, $iOldTrainCostDElixir = 0 ; search and train troops cost
	Local Static $iOldNbrOfOoS = 0 ; number of Out of Sync occurred
	Local Static $iOldNbrOfTHSnipeFails = 0, $iOldNbrOfTHSnipeSuccess = 0 ; number of fails and success while TH Sniping
	Local Static $iOldGoldFromMines = 0, $iOldElixirFromCollectors = 0, $iOldDElixirFromDrills = 0 ; number of resources gain by collecting mines, collectors, drills
	Local Static $iOldAttackedCount = 0, $iOldAttackedVillageCount[$g_iModeCount + 1] ; number of attack villages for DB, LB, TB, TS
	Local Static $iOldTotalGoldGain[$g_iModeCount + 1], $iOldTotalElixirGain[$g_iModeCount + 1], $iOldTotalDarkGain[$g_iModeCount + 1], $iOldTotalTrophyGain[$g_iModeCount + 1] ; total resource gains for DB, LB, TB, TS
	Local Static $iOldNbrOfDetectedMines[$g_iModeCount + 1], $iOldNbrOfDetectedCollectors[$g_iModeCount + 1], $iOldNbrOfDetectedDrills[$g_iModeCount + 1] ; number of mines, collectors, drills detected for DB, LB, TB
	...
	If $iOldNbrOfWallsUppedGold <> Number($iNbrOfWallsUppedGold) Then
		GUICtrlSetData($g_hLblWallGoldMake, Number($iNbrOfWallsUppedGold))
		$iOldNbrOfWallsUppedGold = Number($iNbrOfWallsUppedGold)
		WallsStatsMAJ()
	EndIf

	If $iOldNbrOfWallsUppedElixir <> Number($iNbrOfWallsUppedElixir) Then
		GUICtrlSetData($g_hLblWallElixirMake, Number($iNbrOfWallsUppedElixir))
		$iOldNbrOfWallsUppedElixir = Number($iNbrOfWallsUppedElixir)
		WallsStatsMAJ()
	EndIf
