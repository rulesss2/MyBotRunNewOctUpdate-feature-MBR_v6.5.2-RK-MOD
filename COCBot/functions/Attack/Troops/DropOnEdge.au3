...
Func DropOnEdge($troop, $edge, $number, $slotsPerEdge = 0, $edge2 = -1, $x = -1, $FourFingers = 0) ; adding parameter $FourFinger - DEMEN
	...
	If $number = 1 Or $slotsPerEdge = 1 Then ; Drop on a single point per edge => on the middle
		...
		EndIf
	ElseIf $slotsPerEdge = 2 And $FourFingers = 0 Then ; Drop on 2 points per edge - Adding FourFinger - DEMEN
	...
	Else
		...
		Local $minY = $edge[0][1]
		Local $maxY = $edge[4][1]
		If $FourFingers = 5 Then ; if $nbSide = 5 - FourFinger - DEMEN
			Local $minXTL = $TopLeft[0][0];FourFinger - DEMEN
			Local $maxXTL = $TopLeft[4][0];FourFinger - DEMEN
			Local $minYTL = $TopLeft[0][1];FourFinger - DEMEN
			Local $maxYTL = $TopLeft[4][1];FourFinger - DEMEN
		EndIf
		If $edge2 <> -1 Then
			...
			If $FourFingers = 5 Then ; if $nbSide = 5 - FourFinger - DEMEN
				Local $minX2TR = $TopRight[0][0]; FourFinger - DEMEN
				Local $maxX2TR = $TopRight[4][0]; FourFinger - DEMEN
				Local $minY2TR = $TopRight[0][1]; FourFinger - DEMEN
				Local $maxY2TR = $TopRight[4][1]; FourFinger - DEMEN
			EndIf
		EndIf
		...
		For $i = 0 To $slotsPerEdge - 1
			...
			If $FourFingers = 5 Then ; if $nbSide = 5 - FourFinger - DEMEN
				Local $posX = $minX + (($maxX - $minX) * ($slotsPerEdge - $i)) / ($slotsPerEdge - 1); FourFinger - DEMEN
				Local $posY = $minY + (($maxY - $minY) * ($slotsPerEdge - $i)) / ($slotsPerEdge - 1); FourFinger - DEMEN
				AttackClick($posX, $posY, $nbtroopPerSlot, SetSleep(0), 0, "#0108")					; FourFinger - DEMEN
				Local $posX = $minXTL + (($maxXTL - $minXTL) * $i) / ($slotsPerEdge - 1)			; FourFinger - DEMEN
				Local $posY = $minYTL + (($maxYTL - $minYTL) * $i) / ($slotsPerEdge - 1)			; FourFinger - DEMEN
			Else																					; FourFinger - DEMEN
				Local $posX = Round($minX + (($maxX - $minX) * $i) / ($slotsPerEdge - 1))
				Local $posY = Round($minY + (($maxY - $minY) * $i) / ($slotsPerEdge - 1))
			EndIf																					; FourFinger - DEMEN
			...
			If $edge2 <> -1 Then ; for 2, 3 and 4 sides attack use 2x dropping
				If $FourFingers = 5 Then ;FourFinger - DEMEN
					Local $posX2 = $maxX2 - (($maxX2 - $minX2) * ($slotsPerEdge - $i)) / ($slotsPerEdge - 1);FourFinger - DEMEN
					Local $posY2 = $maxY2 - (($maxY2 - $minY2) * ($slotsPerEdge - $i)) / ($slotsPerEdge - 1);FourFinger - DEMEN
					AttackClick($posX2, $posY2, $nbtroopPerSlot, SetSleep(0), 0, "#0109");FourFinger - DEMEN
					Local $posX2 = $maxX2TR - (($maxX2TR - $minX2TR) * $i) / ($slotsPerEdge - 1);FourFinger - DEMEN
					Local $posY2 = $maxY2TR - (($maxY2TR - $minY2TR) * $i) / ($slotsPerEdge - 1);FourFinger - DEMEN
				Else;FourFinger - DEMEN
